...
//        var colors = ['#ff8762', '#5ebade', '#b298e1', '#70cac1', '#cf2030'];
    ...
    render_department_employee_shift:function(){
        var self = this;
        var w = 200;
        var h = 200;
        var r = h/2;
        var elem = this.$('.emp_graph_shift');
//        var colors = ['#ff8762', '#5ebade', '#b298e1', '#70cac1', '#cf2030'];
        var colors = ['#70cac1', '#659d4e', '#208cc2', '#4d6cb1', '#584999', '#8e559e', '#cf3650', '#f65337', '#fe7139',
        '#ffa433', '#ffc25b', '#f8e54b'];
        var color = d3.scale.ordinal().range(colors);
        rpc.query({
            model: "hr.employee",
            method: "get_dept_employee_shift",
        }).then(function (data) {
            var segColor = {};
            var vis = d3.select(elem[0]).append("svg:svg").data([data]).attr("width", w).attr("height", h).append("svg:g").attr("transform", "translate(" + r + "," + r + ")");
            var pie = d3.layout.pie().value(function(d){return d.value;});
...
//        var colors = ['#ff8762', '#5ebade', '#b298e1', '#70cac1', '#cf2030'];
    ...
    update_join_resign_trends_dep: function(){
        var elem = this.$('.join_resign_trend_dep');
        var colors = ['#70cac1', '#659d4e', '#208cc2', '#4d6cb1', '#584999', '#8e559e', '#cf3650', '#f65337', '#fe7139',
        '#ffa433', '#ffc25b', '#f8e54b'];
        var color = d3.scale.ordinal().range(colors);
        ...
        rpc.query({
            ...
            svg.append("g")
                .attr("class", "x axis")
                .attr("transform", "translate(-47," + height + ")")
                .call(xAxis);
...
//        var color = d3.scale.category10();
        ...
        rpc.query({
                ...
                function pieChart(pD){
                    console.log("pDeeeeeeeeeeeeeeeeeee",pD)
                ...
                //     console.log("deeeeeeeeeeeeeeeekkkkkkkkkkkkkkkkkkkk",d)
                ...
                var hG = histoGram(fData), // create the histogram.
                    pC = pieChart(pie_da), // create the pie-chart.
                    leg= legend(pie_da);  // create the legend.
        });
    },
