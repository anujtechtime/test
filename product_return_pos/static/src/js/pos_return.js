...
var ReturnWidgetProductDetails = PopupWidget.extend({
    template:'ReturnWidgetProductDetails',
    ...
    render_list:function(options){
        ...
        rpc.query({
                ...
                lines = result[0];
                console.log("lines@@3333333333333444444444444455555555555566666666666667",lines)
                for(var j=0;j < lines.length; j++){
                    var product_line = lines[j];
                    var rows = "";
                    console.log("product_line@@@22222222222222@@@@@@@@@@@@@@",product_line)
                    var id = product_line.product_id
                    console.log("iddddddddddddddddddddddddddddddddddddddddd",id)
                    var price_unit = product_line.price_unit;
                    var product =product_line.product;



                    var ref =product_line.internal_ref; 

                    var unit_price = product_line.unit_price;
                    var net_price = product_line.net_price;

                    var margin = product_line.margin.toFixed(2)

                    var sub_total = product_line.qty * net_price;

                    var qty = product_line.qty;
                    var line_id = product_line.line_id;
                    var discount = product_line.discount;
                    totla = totla + sub_total;
                    rows += "<tr><td style='border: 1px solid #ddd;'>" + product + "</td><td style='border: 1px solid #ddd;'>" + ref +" </td><td style='border: 1px solid #ddd;'>" + qty + "</td><td style='border: 1px solid #ddd;'>" + unit_price + "</td><td style='border: 1px solid #ddd;'>" + net_price + "</td><td style='border: 1px solid #ddd;'>" + discount + "</td><td style='border: 1px solid #ddd;'>" + margin + "</td><td style='border: 1px solid #ddd;'>" + sub_total + "</td></tr>";
                    $(rows).appendTo("#lists tbody");
                    var rows = document.getElementById('lists').rows;
                ...
                var table_total = document.getElementById('all_total').innerHTML = totla;
...
gui.define_popup({name:'ReturnWidgetProductDetails', widget: ReturnWidgetProductDetails});
...
var ReturnWidget = PopupWidget.extend({
    ...
    render_list:function(options){
        ...
        rpc.query({
                ...
                for(var j=0;j < lines.length; j++){
                    ...
                    rows += "<tr><td>" + id + "</td><td>" + line_id + "</td><td>" + name +" </td><td>" + ref + "</td><td>" + qty + "</td><td>" + unit_price + "</td><td>" + price_unit + "</td><td>" + discount + "</td><td>" + margin +  "</td></tr>";
    ...
    click_confirm: function(){
        ...
        for (r=0, n = myTable.rows.length; r < n; r++) {
            ...
            if (row.cells[4].innerHTML < return_qty){
                count +=1
            }
            c = c+1
        }
        if (count > 0){
            ...
            for (var r=0, n = myTable.rows.length; r < n; r++) {
                ...
                if (return_qty > 0){
                    ...
                    price: row.cells[6].innerHTML,
                    quantity: -(return_qty),
                    discount:row.cells[7].innerHTML,
                    merge: false,
...
var OrderListScreenWidget = ScreenWidget.extend({
    ...
    init: function(parent, options){
        ...
        this.click_order_line  = function(event){
            console.log("event1111111111111122222222222222",event)
            console.log("nnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnn",this)
            var self = this;
            this.click_order_lines();
            console.log("self22223333333334444444444444455555555555",this)
            // self.gui.show_popup('ReturnWidget',{
    ...
    show: function(){
        ...
        this.$('.Product_details').click(function(e){
            var order = $(e.target).closest("tr").data('id');
            self.order_product_details(order);
        });
    ...
    render_list: function(orders){
        ...
        for(var i = 0, len = Math.min(orders.length,1000); i < len; i++){
            ...
            console.log("orderline.innerHTML@@@@@@@22222222222222222333333333333",orderline.innerHTML)
            orderline = orderline.childNodes[1];

            // orderline.addEventListener('click',this.click_order_line);
            contents.appendChild(orderline);
        }
    },

    click_order_lines: function(product){
        console.log("click_order_line@@@@@@@@@@@@@@@@@@@@@@",click_order_line)
    },

    order_product_details: function(products){
        console.log("products222222222222222222222222222",products)
        var self = this;
        var order = this.get_order_by_id(products);
        console.log("order333333333562637137123818281273712873971293791",order)
        var client = ''
        var client_name = ''
        if (order.partner_id){
             client = order.partner_id[0];
             client_name = order.partner_id[1];
             console.log("")
        }
        self.gui.show_popup('ReturnWidgetProductDetails',{ref: order.pos_reference,client:client, client_name: client_name });
    ...
    return_order:function(order_id){
        ...
        console.log("order@2222222333333333333444444444455555555555",order)
        var client = ''
        if (order.partner_id){
             client = order.partner_id[0];
        }
