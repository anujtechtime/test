    ...
    def schedule_backup(self):
        ...
        for rec in conf_ids:
            ...
            try:
                ...
                return False    
            if rec.autoremove:
                res = s3_client.list_objects(Bucket='database.backup.erp-techtime')
                for resp in res.get("Contents"):
                    if 'LastModified' in resp:
                        LastModified = resp.get('LastModified')
                        _logger.info('(Part of the) path didn\'t exist. Creating it now at ' + str(LastModified))
                        now = datetime.datetime.now()
                        naive = LastModified.replace(tzinfo=None)
                        delta = now - naive
                        if delta.days >= rec.days_to_keep:
                            _logger.info('(responseresponseresponseresponse\'t exist. Creating it now at ' + str(rec.days_to_keep))
                            response = s3_client.delete_objects(
                                Bucket='database.backup.erp-techtime',
                                Delete={
                                    'Objects': [
                                        {
                            ...
                            _logger.info('(Part rec.days_to_keeprec.days_to_keep the) path didn\'t exist. Creating it now at ' + str(response))
