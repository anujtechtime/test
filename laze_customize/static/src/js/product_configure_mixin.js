         ...
         ajax.jsonRpc("/variant_change_images", 'call', {
                    ...
                    if(data.value_ret === 1){
                        ...
                        if($t_slides.attr("data-slider_look") === '1'){
                            ...
                            $.getScript("/laze_customize/static/src/scss/options/product-details/option-1.js");
                            if(data.opt_zoom === 1)
                            ...
                            if(data.sub_img.trim())
                                $t_slides.empty().append(data.sub_img.trim());
                            $.getScript("/laze_customize/static/src/scss/options/product-details/option-2.js");
                            if(data.opt_zoom === 1)
                                $.getScript("/laze_customize/static/src/js/zoom.js");
                            if(data.opt_gallery === 1)
                                $.getScript("/laze_customize/static/src/js/product_image_gallery_js.js");
