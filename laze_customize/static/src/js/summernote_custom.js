odoo.define('laze_customize.embeded_snippet',function(require) {
  'use strict';
  ...
  ajax.loadXML('/laze_customize/static/src/xml/summer_note.xml', core.qweb);
  options.registry.summernote_embeded = options.Class.extend({
    xmlDependencies: ['/laze_customize/static/src/xml/summer_note.xml'],
    start : function () {
      var self = this;
    ...
    modify_note : function(type,value) {
      ...
      if(type == false || type == 'click'){
        var dialog =new Dialog(self, {
          ...
          $content: QWeb.render("laze_customize.embeded_block",{'data':markupStr}),
        });
        dialog.open();
        return self;
      }
