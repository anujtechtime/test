    ...
    def variant_change_images(self, product_id, **kw):
        ...
        if product_id:
            ...
            if product_rec:
                ...
                ret.update({
                    'value_ret': 1,
                    'main_img': request.env['ir.ui.view'].render_template('laze_customize.main_image_box', values),
                    'sub_img': request.env['ir.ui.view'].render_template('laze_customize.sub_images_box', values),
                    'opt_zoom': int(request.website.viewref('laze_customize.product_picture_magnify').active),
                    'opt_gallery': int(request.website.viewref('laze_customize.product_image_gallery').active)
                })
        return ret
